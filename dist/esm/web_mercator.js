import { degreesToRadians, radiansToDegrees } from './model';
 ...
 */
export const pointToWebMercator = (lng, lat) => {
 ...
 */
export const webMercatorToPoint = (x, y) => {
    const lng = radiansToDegrees(x) / EARTH_RADIUS;
    const lat = radiansToDegrees(Math.PI * 0.5 - 2.0 * Math.atan(Math.exp(-y / EARTH_RADIUS)));
    const [longitude, latitude] = truncatePoint(lng, lat);
